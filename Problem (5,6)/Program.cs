using System;

namespace Problem5_6
{
    class Program
    {
        static void Main()
        {
            int n,x;
            int number;
            string world;
            Console.Write("Select a quantity n:");
            n = Convert.ToInt32(Console.ReadLine());
            List<Box<string>> boxes = new List<Box<string>>(n);
            List<Box<int>> boxes2 = new List<Box<int>>(n);

            do
            {
                Console.WriteLine("To display Problem (3): press 1 , if Problem (4): press 2 , to finish: press 3");
                x = Convert.ToInt32(Console.ReadLine());
                switch (x)
                {
                    case 1:
                        for (int i = 0; i < n; i++)
                        {
                            Console.Write("Add:");
                            boxes.Add(new Box<string>(Console.ReadLine()));
                        }
                        Console.Write("Selecet world for check: ");
                        world = Console.ReadLine();

                        Console.WriteLine(Count_Method<string>(boxes, world));
                        break;
                    case 2:
                        for (int i = 0; i < n; i++)
                        {
                            Console.Write("Add:");
                            boxes2.Add(new Box<int>(Convert.ToInt32(Console.ReadLine())));
                        }
                        Console.Write("Selecet world for check: ");
                        number = Convert.ToInt32(Console.ReadLine());

                        Console.WriteLine(Count_Method<int>(boxes2, number));
                        break;

                    default:
                        Console.WriteLine("Press 3 if you want exit");
                        break;
                }
            }
            while (x != 3);

            Console.ReadLine();
        }

        static int Count_Method<T>(List<Box<T>> masiv, T for_check) where T : IComparable<T>
        {
            int add = 0;
            foreach (Box<T> box in masiv)
            {
                if (box.Count(for_check))
                {
                    add++;
                }
            }
            return add;
        }
    }
}